...
- **Icon-only arm** (`IconButtonLinkProps`) is selected by `icon` +
  ...
  would inflate the link into a container-sized square.
...
labelled button-link (disabled collapses to an inert element that keeps the button
styling and its accessible name, out of the a11y tree as a link).
