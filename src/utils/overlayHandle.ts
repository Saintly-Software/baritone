 ...
 * controlled `open` / `onOpenChange`. This is the small escape hatch for
 * closing after an `await` without lifting `open` into component state.
 *
 * A structural view of base-ui's own `DialogHandle` / `PopoverHandle`, so it
 * stays a strict subset of what each overlay's `handle` prop accepts.
 */
export interface OverlayHandle {
  /**
   * Closes the overlay through the same path as every other dismissal: fires
   * `onOpenChange`, respects a controlled `open`, and is vetoed while `disabled`.
   */
  close(): void;
...
/**
 * A baritone overlay that can mint an imperative handle. `Modal`, `Popover`,
 * and `Drawer` each expose a `createHandle`.
 */
 ...
 * alongside it — an addition to the declarative model, not a replacement.
 */
