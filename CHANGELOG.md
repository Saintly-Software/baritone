...
- 9fca7b8: Add `useControlledDrawer` and `useControlledModal` — hooks for driving an
...
- b698997: Add expandable row detail panels to `DataTable` — pass `renderDetailPanel` to give
...
- 46c1d22: Make every `icon` slot accept a **bare glyph**, so a caller no longer hand-wraps
  each one in `<Icon>`:
...
- 140f824: `Icon` now owns its vertical alignment inside text, not just its colour. Flowing
...
- 58fff1b: Make border width a consumer-extensible _open_ vocabulary, mirroring `Text`'s
...
- 0373b87: Expand the `whiteSpace` and `overflowWrap` atoms to the full CSS keyword sets, so
...
- 8efaa95: Give `ToggleGroupItem` `Button`'s first-class icon vocabulary — **`startIcon`**,
...
- e1d8c2b: Add `InlineList` — a horizontal run of items separated by a delimiter that wraps
...
- ae42471: Add `pre-wrap` to the `whiteSpace` atom, so `Text` (and `Heading`, which shares
  the atom) can preserve newlines and runs of spaces in its content while still
  wrapping at the container edge — useful for user-authored copy, log lines, and
  model output.
